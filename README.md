...
v3.37.0 — Sweeping Feature Audit: 5 Real Fixes.
- Ran a three-way parallel audit of every page and every runtime route. Result: the large majority of the app was genuinely complete and correctly wired, no fabricated data or dead buttons found beyond the five items below.
- **Roll-call attendance was a mathematical tautology.** Commit `9bf5cb0` introduced `currentCanonicalParticipants()` (active-only) to fix a real duplicate-row bug, but left `activeParticipants = participants` pointing at that same already-filtered set — so `present === total` always, and the attendance rate could only ever read ~100% (or 0%). This silently broke the Dashboard attendance stat, the "low attendance" alert (which could never fire), Agent Chat's attendance reply, and the Agent success rate. New `allCanonicalParticipants()` (`communication/roll-call.js`) restores a real active/total ratio while keeping the legitimate duplicate-row exclusion on both sides — a genuine improvement over the pre-regression version, not just a revert. Verified live: a real roll call now reads `8/12 · 66.7%`, not a hardcoded 100%.
- **Participants' header "select all" checkbox was dead code** — `updateBulkBar()` ended with `&& false`, unconditionally forcing it unchecked regardless of real selection state. Now genuinely reflects whether every visible row's checkbox is checked.
- **Message Activity was fake.** `DATA.messageActivity` (Dashboard + Analytics) was a fixed 12-entry seed array, never recomputed — unlike the sibling `totalMessages`, which genuinely is kept live. New `computeMessageActivity()` (`app/charts.js`) buckets every real message's real timestamp into genuine hour-of-day activity.
- **Sentiment honestly relabeled, not fabricated.** No sentiment-analysis capability exists anywhere in this codebase — inventing a keyword-scoring heuristic now would itself be a fabrication. The panel keeps its existing seed values but is now clearly labeled "(illustrative — not computed from real message content)," matching the app's own `PROPOSED`/`WORKING` honesty-labeling pattern elsewhere.
- **Found and fixed a real, silent auth bug** while reviewing the Admin Runtime Backend panel: both token reads in `app/pages/admin.js` used `localStorage`, but the token is only ever written to `sessionStorage` — every request from that panel and "Backup Runtime Now" was silently unauthenticated, returning real `401`s rendered as if partial/empty data rather than an obvious crash. Fixed to match every other real reader in the app.
- **4 real, tested, never-called-from-any-UI backend routes** (`GET /api/tasks`, `/handoffs`, `/chat`, `/commands`) now have a genuine caller: the Admin Runtime Backend panel shows "Recent Tasks/Handoffs/Chat/Commands," pulling real synced records live from the runtime.
- Verified: 320/320 unit tests (up from 309, two new suites — `communication.roll-call.test.js`, `app.charts.test.js`), live browser regression on all five fixes with zero console errors, including the Admin panel now showing real authenticated data (81 real chat messages, 278 real command-audit entries) instead of silently failing.

v3.36.0 — Tool Readiness Ticket → Real Tool Enablement Verification.
- Audited "tools and modules" completeness across three areas: all 66 identities' `memberProfile.toolProfile.assignedTools` were already complete, and `data/team-working-profiles.js`'s `approvedToolClasses` was already complete and consistent for all 66 — no gap, no fabricated work needed there. The real gap: the Tool Readiness ticket workflow (`buildToolReadinessTickets`, wired to a real "🧰 Build Tool Tickets" button) generated a complete per-identity tool inventory, but the ticket detail view never displayed it, and resolving a ticket had no linkage back to the Knowledge Path `tools` stage — unlike the parallel, fully-wired Competency Baseline pilot.
- `app/pages/helpdesk.js`: each generated ticket now carries a real `toolReadinessReview` (mirroring `knowledgePathPilot`'s shape) with `@HELIX` as the independent reviewer — matching the reviewer the ticket's own acceptance-criteria text already named, not invented. New `submitToolReadinessEvidence()` / `reviewToolReadinessEvidence()` reuse the same hardened `recordKnowledgePathStageEvidence()` from v3.35.0, so approving a ticket is real evidence-gated verification of the `tools` stage, not a bare status flip — rejection stays `REVIEWER_MISMATCH`/`REVIEW_NOTE_REQUIRED`-guarded exactly like the pilot.
- Ticket detail view now renders the full assigned-tool inventory and the review workflow state/actions, closing the "generated but invisible" gap.
- Verified: 309/309 unit tests (up from 299), live browser regression building 66 real tickets, submitting evidence, and an `@HELIX` approval genuinely resolving the ticket with zero console errors.
